...
@ApiTags('Link publications')
@Controller(`publications/${POSTFIX}`)
@UseFilters(AxiosExceptionFilter)
export class LinkPublicationsController {
  constructor(
  ...
  public async remove(@Param('id') id: string, @Req() { user }: RequestWithTokenPayload) {
    const { data } = await this.httpService.axiosRef.get<LinkPublicationRDO>(`${this.appConfig.url.publications}/${POSTFIX}/${id}`);
